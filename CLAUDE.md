...
- **BUILD_ID**: Bundle contenthash from webpack build
  - Same hash used in bundle filename (e.g., `bundle.492e859ac8d78db9c881.js`)
  - Browser extracts hash from script src
  - Server reads hash from `dist/bundle-hash.json` (generated by webpack)
  - Ensures browser and server are using the same deployment

**How It Works:**
1. **Build Time**: Webpack builds `bundle.HASH.js` and writes hash to `dist/bundle-hash.json`
2. **Browser Load**: Browser extracts BUILD_ID from bundle script src (bundle.HASH.js)
3. **Server Start**: Server reads BUILD_ID from `dist/bundle-hash.json`
4. **Registration**: Client sends `protocolVersion` and `buildId` in register message
5. **Validation**: Bootstrap server checks both values:
   - Protocol version must be compatible (major.minor match)
   - Build ID must match (both extracted from same webpack build)
6. **Rejection**: On mismatch, server sends `version_mismatch` message with refresh instruction
7. **Refresh**: User refreshes browser → nginx serves new bundle.HASH.js with matching hash
8. **Connection**: Reconnection succeeds with matching versions
...
```json
{
  ...
  "clientVersion": "1.0.0",
  "clientBuildId": "492e859ac8d78db9c881",
  "serverVersion": "1.0.0",
  "serverBuildId": "abc123def456...",
  "message": "Bundle version mismatch. Please refresh your browser to get the latest version."
}
```

**Implementation Files:**
- `src/version.js` - Version constants, BUILD_ID extraction, and compatibility checking
...
- `webpack.config.cjs` - Writes bundle hash to `dist/bundle-hash.json` after build
- `dist/bundle-hash.json` - Generated file containing `{ "hash": "...", "timestamp": ... }`

**BUILD_ID Extraction:**
```javascript
// Browser: Extract from script src
const scripts = document.getElementsByTagName('script');
for (const script of scripts) {
  const match = script.src.match(/bundle\.([a-f0-9]+)\.js/);
  if (match) BUILD_ID = match[1];
}

// Server: Read from bundle-hash.json
const data = JSON.parse(fs.readFileSync('dist/bundle-hash.json'));
BUILD_ID = data.hash;
```

**Fallback Handling:**
- If `bundle-hash.json` doesn't exist (server started before build), uses `node_TIMESTAMP`
- If browser can't find bundle script, uses `unknown_browser`
- Version checking is skipped when either side has a fallback BUILD_ID
